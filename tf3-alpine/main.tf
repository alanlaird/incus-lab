...
#
# On first apply the script runs ssh-keygen to create alpine_id_ed25519{,.pub}.
...
data "external" "ssh_key" {
  program = ["bash", "-c", <<-SCRIPT
    KEY="${path.module}/alpine_id_ed25519"
    if [ ! -f "$KEY" ]; then
...
resource "local_file" "ansible_inventory" {
  depends_on = [incus_instance.alpine]
  filename   = "${path.module}/ansible/inventory.ini"
  content    = <<-EOT
    [alpine_vms]
    ${join("\n", [
  for vm in incus_instance.alpine :
  "${vm.name} ansible_host=${coalesce(vm.ipv4_address, "pending")} ansible_user=root ansible_ssh_private_key_file=${abspath(path.module)}/alpine_id_ed25519"
])}
